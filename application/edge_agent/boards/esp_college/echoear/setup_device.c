...
static const char *TAG = "echoear_board";
                                    ...
                                    esp_lcd_panel_handle_t *ret_panel)
{
    ...
    esp_err_t ret = esp_lcd_new_panel_st77916(io, &panel_dev_cfg, ret_panel);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "New ST77916 panel failed: %s", esp_err_to_name(ret));
        return ret;
    }
    return ESP_OK;
}

static IRAM_ATTR void cst816s_interrupt_cb(esp_lcd_touch_handle_t tp)
{
...
{
    static SemaphoreHandle_t s_cst816s_int_sem = NULL;
    esp_lcd_touch_config_t touch_cfg = {0};
    memcpy(&touch_cfg, touch_dev_config, sizeof(esp_lcd_touch_config_t));

    if (s_cst816s_int_sem == NULL) {
        s_cst816s_int_sem = xSemaphoreCreateBinary();
    }
    touch_cfg.interrupt_callback = cst816s_interrupt_cb;
    touch_cfg.user_data = (void *)s_cst816s_int_sem;
